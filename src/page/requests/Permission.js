...
const Permission = ({ navigation }) => {
  ...
  const isFormValid = () => {
    let exceptions = ['time']; // add any field you want to skip validation for
    for (const [key, value] of Object.entries(form)) {
      if (exceptions.includes(key)) continue;

      if (value === '' || value === null) {
        //console.warn(`Field "${key}" is required.`);
        Alert.alert(`Field "${key}" is required.`);
        return false;
      }
    }
    return true;
  ...
  const isImgInputValid = () => {
    if (form.type != 'sick_proof') {
      return true;
    } else {
      for (const [key, value] of Object.entries(imageP)) {
        if (value === '' || value === null) {
          //console.warn(`Field "${key}" is required.`);
