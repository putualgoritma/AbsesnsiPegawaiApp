...
const ListAbsence = ({navigation, route}) => {
  ...
  const handleAction = id => {
    Alert.alert('Yakin ?', `mengakhiri Izin/Cuti`, [
      {
        text: 'Yakin',
        onPress: () => {
          setLoading(true);
      ...
      <ScrollView
        ...
        {!loading && (
          <View>
            ...
            {data.menu.menuLeave == 'ON' && data.leave.type!='sick' && (
            ...
            {data.menu.menuLeave == 'ON' && data.leave.type=='sick' && (
              ...
              <TouchableOpacity
                  style={[styles.listMenu, {backgroundColor: '#7a8793'}]}
                  onPress={() => {
                    handleAction(data.leave.id);
                  }}>
                  <Text style={styles.btnText}>Akhiri Cuti Sakit?</Text>
                </TouchableOpacity>
            )}
